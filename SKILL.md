...
앱 검증 시나리오는 기능 목록이 아니라 실제 사용자 흐름 기준으로 작성한다.
각 시나리오는 사용자가 어떤 상태에서 시작해 어떤 행동을 하고, 앱이 어떤 화면/상태/데이터 변화로 반응해야 하는지 검증해야 한다.
React Native 앱 검증에서는 Maestro로 실행 가능한 형태를 기준으로 작성한다.
각 시나리오에는 계정/권한 상태, app build, platform, simulator/device, selector 전략, 필요한 `testID`/accessibility id를 명시한다.
시나리오는 다음 범위를 포함한다.
1. 핵심 happy path.
2. 입력/권한/네트워크/비동기/저장 상태 관련 edge case.
3. 새 기능과 연관된 기존 사용자 흐름의 regression scenario.
4. 공유 컴포넌트/API/상태/권한이 깨지지 않았는지 확인하는 adjacent scenario.
5. 현재 환경에서 검증할 수 없는 항목과 이유.
...
1. Metadata:
   1. User goal:
   2. Coverage type: Happy path | Edge | Regression | Adjacent
   3. Source: User-requested | AI-proposed
   4. Priority: Required | Recommended | Optional
   5. Platform coverage: iOS simulator | Android real device | iOS simulator + Android real device | Web | N/A
   6. Status: Pending | Passed | Failed | Blocked
2. Environment:
   1. Start state:
   2. Account/auth state:
   3. Permission state:
   4. Network/mock/secret state:
   5. Browser/auth state 또는 simulator/device/auth state:
   6. Viewport 또는 simulator/device/app build:
   7. Maestro flow 또는 headed Playwright 방법:
3. Selector Strategy:
   1. 사용할 `testID`/accessibility id/id/text selector:
   2. 없는 `testID`와 plan에 반영해야 할 testability gap:
   3. 좌표 기반 제스처 사용 여부와 이유:
4. Steps:
   1. Step 1.
   2. Step 2.
   3. Step 3.
5. Expected Result:
   ...
   4. Console/network checks:
   5. Refresh/back-navigation checks:
...
답변된 질문은 답변을 `Verification Environment`, `App Scenario Design Rules`, `Scenario Details`, `Excluded Scenarios` 등 적절한 섹션에 반영한 뒤 제거한다.
